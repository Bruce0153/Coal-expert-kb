...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Rerank top_n (docs fed to LLM). Overrides config.retrieval.rerank_top_n.",
    )
    ...
    if backend in {"elastic", "both"}:
        ...
        # ✅ only k candidates are retrieved from ES too
    ...
    while True:
        ...
        docs_for_llm = docs[:rerank_top_n] if (args.llm and rerank_enabled) else docs
        ans = answerer.answer(q, docs_for_llm)
        print("\n" + ans)
