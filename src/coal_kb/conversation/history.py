...
def _is_follow_up(query: str) -> bool:
    ...
    return any(
        token in normalized
        for token in ["what about", "how about", "compare", "那", "继续", "对比"]
    )
...
def prepare_history_context(
    ...
    *,
    ...
    recent_user_messages = [message.content for message in recent_messages if message.role == "user"][-2:]
