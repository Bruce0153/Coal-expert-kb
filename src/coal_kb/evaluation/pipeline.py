...
from coal_kb.evaluation.metrics import aggregate_metrics, answer_metrics, retrieval_metrics
...
RetrieveFunction = Callable[[EvaluationCase, int], list[Document]]
...
class EvaluationPipeline:
    ...
    def process(self) -> dict[str, Any]:
        ...
        for case in tqdm(cases, total=len(cases), desc=self.__class__.__name__):
            ...
            documents = self.retrieve_fn(case, max_k)
            retrieved = tuple(
            ...
            failure = classify_failure(case, retrieval_values, answer_values)
            results.append(
