...
from coal_kb.ingestion.cleaning.text import (
...
from coal_kb.ingestion.loaders.base import BaseLoader, detect_language
from coal_kb.ingestion.loaders.registry import register_loader
from coal_kb.ingestion.parsing.pdf_loader import load_pdf_pages
...
class PDFLoader(BaseLoader):
    ...
    def _column_order(self, lines: List[_LineSpan]) -> List[_LineSpan]:
        ...
        if not is_two_col:
            return sorted(lines, key=lambda t: (t.y, t.x))
        mid = (xmin + xmax) / 2
        left = sorted([item for item in lines if item.x <= mid], key=lambda t: (t.y, t.x))
        right = sorted([item for item in lines if item.x > mid], key=lambda t: (t.y, t.x))
        return left + right
