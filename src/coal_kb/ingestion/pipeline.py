...
from coal_kb.ingestion import config
from coal_kb.utils.file_hash import sha256_file
from coal_kb.utils.hash import stable_chunk_id
from coal_kb.utils.unicode_sanitize import sanitize_obj
...
def _embed_documents_resilient(
    ...
    *,
    ...
    min_batch_size: int = config.EMBEDDING_MIN_BATCH_SIZE,
) -> List[List[float]]:
...
class IngestPipeline:
    ...
    def process(
        ...
        *,
        ...
        for path_str, file_path in tqdm(
            file_items, total=len(file_items), desc=self.__class__.__name__
        ):
            stat = file_path.stat()
        ...
        pages_with_T = 0
        pages_with_P = 0
        for d in tqdm(page_docs, total=len(page_docs), desc=self.__class__.__name__):
        ...
        for i, ch in enumerate(tqdm(chunks, total=len(chunks), desc=self.__class__.__name__)):
        ...
        if store:
            stage_start = time.monotonic()
            batch_size = config.VECTOR_WRITE_BATCH_SIZE
    ...
    def run(
        ...
        *,
        ...
        return self.process(
            rebuild=rebuild,
            force=force,
            elastic_index_override=elastic_index_override,
        )
