from coal_kb.answering import AnswerResult
from coal_kb.application.ask import AskExecution
...
def test_chat_orchestrator_returns_conversation_capable_response(tmp_path, monkeypatch):
    ...
    def fake_execute_query(*args, **kwargs):
        return AskExecution(
            ...
            result=AnswerResult(
                ...
                citations={"E1": {"label": "E1", "source_file": "paper.pdf", "title": "paper.pdf", "page": 2, "heading_path": "Results", "chunk_id": "c1", "snippet": "CO2 example", "source_display": "paper.pdf | page 2 | Results", "source_id": "paper|paper.pdf"}},
                used_chunks=["c1"],
                evidence_items=[{"label": "E1", "source_file": "paper.pdf", "title": "paper.pdf", "page": 2, "heading_path": "Results", "chunk_id": "c1", "snippet": "CO2 example", "source_display": "paper.pdf | page 2 | Results", "source_id": "paper|paper.pdf"}],
                source_cards=[{"source_id": "paper|paper.pdf", "source_file": "paper.pdf", "title": "paper.pdf", "pages": [2], "headings": ["Results"], "evidence_labels": ["E1"], "evidence_count": 1, "snippet_preview": "CO2 example"}],
                ...
                referenced_labels=["E1"], evidence_sufficiency="limited", confidence_score=0.7, debug={},
            ),
            timings_ms={"total": 1.0}, history_used=True, history_reason="follow_up_rewrite",
        )

    monkeypatch.setattr("coal_kb.application.chat.execute_query", fake_execute_query)
    monkeypatch.setattr("coal_kb.application.chat.log_query", lambda *args, **kwargs: None)
    ...
    second = orchestrator.chat(query="What about CO2 instead?", conversation_id=first.conversation.conversation_id, enable_llm=False)
