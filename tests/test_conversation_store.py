...
def test_conversation_creation_and_message_persistence(tmp_path):
    ...
    user_message = service.add_message(conversation_id=conversation.conversation_id, role="user", content="How does steam gasification affect NH3?")
    assistant_message = service.add_message(conversation_id=conversation.conversation_id, role="assistant", content="It depends on temperature.", metadata={"confidence_score": 0.5})
