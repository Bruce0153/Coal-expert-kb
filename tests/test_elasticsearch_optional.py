...
def test_elastic_retriever_roundtrip(monkeypatch):
    ...
    docs = [
        {"chunk_id": "c1", "document_id": "d1", "source_file": "a.pdf", "page": 1, "section": "results", "chunk_index": 0, "text": "Steam gasification produces NH3.", "stage": "gasification", "embedding": [0.1, 0.2, 0.3, 0.4]},
        {"chunk_id": "c2", "document_id": "d2", "source_file": "b.pdf", "page": 2, "section": "results", "chunk_index": 1, "text": "Pyrolysis produces tar.", "stage": "pyrolysis", "embedding": [0.2, 0.1, 0.4, 0.3]},
    ]
    ...
    monkeypatch.setattr("coal_kb.infra.persistence.search.make_embeddings", lambda _cfg: DummyEmbeddings())
    factory = store.make_retriever_factory(index=index_name, embeddings_cfg=EmbeddingsConfig(provider="openai", base_url="http://localhost", api_key_env="DUMMY", model="dummy", dimensions=4), candidates=10)
