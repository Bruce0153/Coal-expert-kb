...
def test_ingest_drops_references(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("coal_kb.ingestion.pipeline.PDFLoader.load", fake_pdf_load)
    monkeypatch.setattr("coal_kb.ingestion.pipeline.make_embeddings", lambda cfg: DummyEmbeddings())
    monkeypatch.setattr("coal_kb.infra.persistence.vector.make_embeddings", lambda cfg: DummyEmbeddings())
    captured = {"docs": []}

    def fake_add_documents(self, docs, ids=None):
        captured["docs"].extend(docs)

    monkeypatch.setattr("coal_kb.infra.persistence.vector.ChromaStore.add_documents", fake_add_documents)
    cfg = load_config()
