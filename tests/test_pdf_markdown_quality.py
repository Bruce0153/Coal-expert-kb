...
def test_heading_infer_from_dict(monkeypatch, tmp_path) -> None:
    class FakePage:
        def get_text(self, mode):
            assert mode == "dict"
            return {"blocks": [{"type": 0, "lines": [{"bbox": [10, 10, 100, 20], "spans": [{"text": "Main Title", "size": 20}]}, {"bbox": [10, 40, 100, 50], "spans": [{"text": "Section", "size": 16}]}, {"bbox": [10, 70, 200, 80], "spans": [{"text": "body text", "size": 11}]}]}]}
...
def test_pdf_markdown_fallback_to_text(monkeypatch, tmp_path) -> None:
    pdf = tmp_path / "a.pdf"
    pdf.write_bytes(b"%PDF")
    monkeypatch.setattr("coal_kb.ingestion.loaders.pdf_loader.PDFLoader.extract_pdf_markdown", lambda self, p: (_ for _ in ()).throw(RuntimeError("x")))
    from langchain_core.documents import Document
    monkeypatch.setattr("coal_kb.ingestion.loaders.pdf_loader.load_pdf_pages", lambda p: [Document(page_content="fallback", metadata={"source_file": str(p)})])
